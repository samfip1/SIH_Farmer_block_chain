  ...
  return (
    <section aria-labelledby="dashboard-title" className="space-y-4">
      ...
      >
        ...
        {filtered.map((r) => (
          <div key={r.id} role="row" className="grid lg:grid-cols-10 grid-cols-2 border-t border-border">
            ...
            <div className="px-3 py-2">{r.quantity} {r.unit}</div>
            ...
            <div className={`px-3 py-2 font-semibold ${
              ...
              r.status === "responded" ? "text-primary" : "text-accent-foreground"
            }`}>
              {r.status}
            </div>
            <div className="px-3 py-2">{r.apmcResponse ?? "-"}</div>
            <div className="px-3 py-2">
              <div className="flex flex-wrap items-center gap-2">
                <Link className="text-primary underline underline-offset-4" to={`/requests/${r.id}`}>
                  View
                </Link>
                <button
