...
// Create APMC update
export const createAPMCUpdate = async (req, res) => {
    try {
...
export const getAPMCUpdates = async (req, res) => {
    try {
        const { page = "1", limit = "10", produceUniqueId, apmcUniqueId, sortBy = "updatedAt", sortOrder = "desc", } = req.query;
        ...
        const where = {};
        if (produceUniqueId) {
            where.produceUniqueId = produceUniqueId;
        }
        if (apmcUniqueId) {
            where.apmcUniqueId = apmcUniqueId;
        }
...
export const getAPMCUpdateById = async (req, res) => {
    try {
        const { id } = req.params;
...
// Update APMC update
// Update APMC update
export const updateAPMCUpdate = async (req, res) => {
    try {
        const { id } = req.params;
...
// Delete APMC update
export const deleteAPMCUpdate = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.id;
        // --- FIX: Check for 'id' to prevent passing 'undefined' to Prisma ---
...
// Get my APMC updates
export const getMyAPMCUpdates = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: "User not authenticated" });
...
//# sourceMappingURL=apmcUpdateController.js.map