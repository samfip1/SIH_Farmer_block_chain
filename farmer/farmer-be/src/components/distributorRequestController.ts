...
export class DistributorRequestController {
  ...
  static async updateRequestStatus(req: AuthRequest, res: Response) {
    try {
      const { id } = req.params
      const userId = req.user?.userId
  ...
  static async getRequestStats(req: AuthRequest, res: Response) {
    try {
      ...
      const [receivedPending, receivedAccepted, receivedRejected, sentPending, sentAccepted, sentRejected] =
        await Promise.all([
  ...
  static async getAvailableDistributors(req: AuthRequest, res: Response) {
    try {
      const userId = req.user?.userId
      if (!userId) {
        return res.status(401).json({ error: "Unauthorized" })
