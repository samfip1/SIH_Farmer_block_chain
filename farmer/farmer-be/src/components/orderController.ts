...
// --- FIX: Updated AuthRequest to include role for permission checks ---
interface AuthRequest extends Request {
  user?: {
    userId: string
    role: "ADMIN" | "FARMER" | "DISTRIBUTOR" // Add other roles as needed
  }
}
  ...
  static async getOrders(req: AuthRequest, res: Response) {
    try {
      ...
      if (status && status !== "ALL") {
        where.status = status as any // Cast because status is a string query param
      }
      if (search) {
        where.OR = [
